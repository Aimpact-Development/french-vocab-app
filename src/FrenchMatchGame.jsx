...
const TOPIC_CEFR = {
  ...
  "Emotions & Opinions":"B1","Technology":"B1","Business Terminology":"B2",
};
...
function parseCSV(text) {
  ...
  return lines.slice(1).filter(l => l.trim()).map(line => {
    ...
    if (isNew) return { cefr: c[0]?.trim()||'A1', type: c[1]?.trim()||'W', topic: c[2]?.trim()||'', grammar_type: c[3]?.trim()||'', english: c[4]?.trim()||'', french: c[5]?.trim()||'', example: c.slice(6).join(',').trim() };
    const topic = c[0]?.trim()||'';
    return { cefr: TOPIC_CEFR[topic]||'A1', type:'W', topic, grammar_type: c[1]?.trim()||'', english: c[2]?.trim()||'', french: c[3]?.trim()||'', example: c.slice(4).join(',').trim() };
  });
}
...
const TOPIC_COLORS = {
  "People & Family":"#6366f1","Daily Verbs":"#f59e0b","Home":"#10b981",
  "Food & Drink":"#ef4444","Travel & City":"#3b82f6","Time":"#8b5cf6",
  "Work & Study":"#06b6d4","Health & Body":"#f97316","Weather & Nature":"#22c55e",
  "Adjectives":"#e879f9","Connectors":"#94a3b8","Shopping & Money":"#f43f5e",
  "Emotions & Opinions":"#a78bfa","Technology":"#38bdf8","Common Phrases":"#fb923c",
  "Business Terminology":"#f59e0b",
};

const GRAMMAR_COLORS = {
  "noun":"#60a5fa","verb":"#f59e0b","adjective":"#e879f9","adverb":"#34d399",
  "conjunction":"#94a3b8","preposition":"#fb923c","phrase":"#a78bfa",
};
...
// ── Selection screen ──────────────────────────────────────────────────────────
function SelectionScreen({ onStart, vocab, theme, onToggleTheme }) {
  ...
  const allTopics = useMemo(() => {
    const map = {};
    vocab.filter(w => selCefr.has(w.cefr) && w.type === contentType)
      ...
      <div style={ss.card}>
        ...
        <div style={ss.sectionLabel}>
          ...
          <button style={ss.toggleAll} onClick={() =>
            setSelCefr(prev => prev.size===availCefr.length ? new Set() : new Set(availCefr.map(m=>m.code)))
          }>{selCefr.size===availCefr.length ? 'Deselect all' : 'Select all'}</button>
        </div>
        ...
        <div style={ss.typeToggle}>
          {[{code:'W',label:'Words'},{code:'P',label:'Phrases'},{code:'S',label:'Sentences'}].map(({code,label}) => (
        ...
        <div style={ss.sectionLabel}>
          <span>Topic</span>
          <button style={ss.toggleAll} onClick={() =>
            setSelTopics(prev => prev.size===Object.keys(allTopics).length ? new Set() : new Set(Object.keys(allTopics)))
          }>{selTopics.size===Object.keys(allTopics).length ? 'Deselect all' : 'Select all'}</button>
        </div>
        <div style={ss.grid}>
        ...
        <div style={{ ...ss.sectionLabel, marginTop:20 }}>
          ...
          <button style={ss.toggleAll} onClick={() =>
            setSelGrammar(prev => prev.size===Object.keys(allGrammar).length ? new Set() : new Set(Object.keys(allGrammar)))
          }>{selGrammar.size===Object.keys(allGrammar).length ? 'Deselect all' : 'Select all'}</button>
        </div>
        <div style={ss.grid}>
...
const ss = {
  page: {
    minHeight:'100vh', background:'var(--fm-bg-page)',
  ...
  skinBtn: {
    ...
    borderRadius:5, padding:'6px 12px', fontSize:16, cursor:'pointer',
    color:'var(--fm-text-2)', fontFamily:'inherit',
  },
  sectionLabel: {
    display:'flex', justifyContent:'space-between', alignItems:'center',
    fontSize:11, fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase',
    color:'var(--fm-text-4)', marginBottom:10,
  },
  toggleAll: {
  ...
  typeToggle: {
    display:'flex', background:'var(--fm-bg-input)', borderRadius:6,
...
export default function FrenchMatchGame() {
  ...
  const startRound = useCallback((used, voc) => {
    ...
    const words  = picked.map(i => ({ fr:voc[i].french, en:voc[i].english, cat:voc[i].topic, grammar:voc[i].grammar_type, example:voc[i].example }));
  ...
  useEffect(() => {
    if (roundWords.length > 0 && matched.size === roundWords.length && !transitioning) {
      ...
      setTimeout(() => { questionStartRef.current = Date.now(); startRound(usedIds, activeVocab); }, 1800);
    }
  ...
  if (screen === 'select') return <SelectionScreen onStart={handleStart} vocab={vocab} theme={theme} onToggleTheme={toggleTheme} />;
    ...
    return (
      <div style={gs.page}>
        <div style={gs.doneCard}>
          ...
          <h1 style={{ color:'var(--fm-text-1)', fontSize:28, margin:'0 0 8px', fontWeight:700 }}>Félicitations !</h1>
          <p style={{ color:'var(--fm-text-2)', margin:'0 0 32px', fontSize:16 }}>You matched all {totalWords} words!</p>
          ...
          <div style={{ display:'flex', gap:12, justifyContent:'center' }}>
            <button style={gs.secondaryBtn} className="btn3d" onClick={() => { resetGame(); setGameOver(false); setScreen('select'); }}>Change words</button>
            <button style={gs.restartBtn} className="btn3d" onClick={() => { resetGame(); setGameOver(false); questionStartRef.current=Date.now(); startRound(new Set(), activeVocab); }}>Play again</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div style={gs.page}>
      <div style={gs.progressTrack}><div style={{ ...gs.progressFill, width:progressPct+'%' }} /></div>
      ...
      <div style={gs.header}>
        ...
        <div style={gs.headerRight}>
          ...
          <button style={{ ...gs.menuBtn, ...(autoMode ? gs.autoBtnOn : {}) }} className="btn3d"
            onClick={() => { if (autoMode) window.speechSynthesis?.cancel(); setAutoMode(a=>!a); }}
            title={autoMode ? 'Stop auto-play' : 'Auto-play'}>
            {autoMode ? '⏹ Stop' : '▶ Auto'}
          </button>
          <button style={gs.menuBtn} className="btn3d" onClick={toggleTheme} title="Toggle theme">
            {theme==='dark' ? '☀' : '🌙'}
          </button>
          <button style={gs.menuBtn} className="btn3d" onClick={() => { resetGame(); setGameOver(false); setScreen('select'); }} title="Back">⚙</button>
        </div>
      </div>

      <div style={gs.instructions}>
        Highlighted <span style={{ color:'#fb923c', fontWeight:600 }}>English</span> word is active — click its <span style={{ color:'#818cf8', fontWeight:600 }}>French</span> match.
          ...
          {enOrder.map((wordId, slot) => {
            ...
            return (
              <div key={slot} style={enCardStyle(isActive, isMatched)}>
                ...
                {isMatched && <span style={gs.speakerBtn} onClick={() => speak(word?.fr)} title="Hear French">🔊</span>}
              </div>
            );
          })}
        </div>
            ...
            return (
              <button key={slot} style={frCardStyle(isMatched, isWrong)} className="btn3d" onClick={() => handleFr(slot)} disabled={isMatched}>
                ...
                <div style={gs.frCardInner}>
                  ...
                  {isMatched && word?.example && <span style={gs.exampleText}>{word.example}</span>}
                </div>
      ...
      {transitioning && <div style={gs.roundBanner}>✨ Round complete! Loading next…</div>}
    </div>
  );
}
