...
async function loadVocab() {
  ...
  // Strip UTF-8 BOM if present (Excel's "CSV UTF-8" format adds one)
...
function parseCSV(text) {
  ...
  return lines.slice(1).map(line => {
    ...
    // cols: [topic, grammar_type, english, french, ...example parts]
    // Join cols[4+] back with commas in case the example sentence contained commas
    return {
      topic:        cols[0] ?? '',
      grammar_type: cols[1] ?? '',
      english:      cols[2] ?? '',
...
function SelectionScreen({ onStart, vocab }) {
  ...
  const allTopics = useMemo(() => {
    const map = {};
    vocab.forEach(w => { map[w.topic] = (map[w.topic] || 0) + 1; });
    return map;
  }, [vocab]);

  const allGrammar = useMemo(() => {
    const map = {};
    vocab.forEach(w => { map[w.grammar_type] = (map[w.grammar_type] || 0) + 1; });
    return map;
  }, [vocab]);
    ...
    <div style={ss.page}>
      <div style={ss.card}>
        ...
        <div style={ss.sectionLabel}>
          <span>Topic</span>
          <button style={ss.toggleAll} onClick={toggleAllTopics}>
        ...
        <div style={{ ...ss.sectionLabel, marginTop: 20 }}>
          ...
          <button style={ss.toggleAll} onClick={toggleAllGrammar}>
        ...
        <div style={{ ...ss.sectionLabel, marginTop: 24 }}>
          <span>Max words per session</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 4 }}>
          ...
          <span style={{ fontSize: 13, color: '#64748b' }}>
            {filteredCount > wordLimit
              ...
              : `${filteredCount} word${filteredCount !== 1 ? 's' : ''} will be played`}
          </span>
        </div>
        ...
        <div style={ss.footer}>
          ...
          <button
            ...
            onClick={() => onStart(selTopics, selGrammar, wordLimit)}
          >
            Start Game →
          </button>
        </div>
        {filteredCount > 0 && filteredCount < ROUND_SIZE && (
          <div style={ss.warn}>Select at least {ROUND_SIZE} words to play.</div>
        )}
      </div>
    </div>
  );
...
export default function FrenchMatchGame() {
  ...
  function handleStart(selTopics, selGrammar, wordLimit) {
  ...
  // Keep a ref to the latest handleFr so the auto effect can call it without stale closure
  useEffect(() => { handleFrRef.current = handleFr; }, [handleFr]);

  // Auto-play: EN finishes speaking → FR starts → FR finishes → click correct card.
  // Full sequential chain driven by onend events so nothing overlaps.
  useEffect(() => {
    ...
    function speakFrench() {
      ...
      timer = setTimeout(doClick, 6000); // fallback
    }
    ...
    timer = setTimeout(speakFrench, 6000); // fallback if onend doesn't fire
      ...
      <div style={gs.page}>
        <div style={gs.doneCard}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
            <button style={gs.secondaryBtn} onClick={() => {
              resetGame(); setGameOver(false); setScreen('select');
            }}>
              Change words
            </button>
  ...
  return (
    <div style={gs.page}>
      ...
      <div style={gs.header}>
        ...
        <div style={gs.headerRight}>
          ...
          <button
            ...
            onClick={() => {
              if (autoMode) window.speechSynthesis?.cancel();
              setAutoMode(a => !a);
            }}
          ...
          <button style={gs.menuBtn} onClick={() => {
            resetGame(); setGameOver(false); setScreen('select');
          }} title="Back to selection">⚙</button>
        </div>
      </div>
      ...
      <div style={gs.board}>
        {/* English column */}
        <div style={gs.column}>
          <div style={gs.colHeader}>
            <span style={{ color: '#fb923c' }}>English</span>
          </div>
        ...
        <div style={gs.column}>
          <div style={gs.colHeader}>
            <span style={{ color: '#818cf8' }}>Français</span>
          </div>
          {frOrder.map((wordId, slot) => {
            ...
            return (
              ...
              >
                ...
                {isMatched && (
                  <span style={{ ...gs.checkMark, color }}> ✓</span>
                )}
              </button>
            );
          })}
